...
class NotesService {
  ...
  async addNote({
    ...
    // query to database postgres
    const query = {
      text: 'INSERT INTO notes VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id',
      values: [id, title, body, tags, owner, createdAt, updatedAt],
    };

    // store the result from pool query
    const result = await this._pool.query(query);

    // varification the result
    if (!result.rows[0].id) {
  ...
  async getNotes(owner) {
    try {
      // mendapatkan catatan di cache
      const result = await this._cacheService.get(`notes:${owner}`);
      return JSON.parse(result);
    } catch (error) {
      // bila gagal, diteruskan dengan mendapatkan catatan di database
      const query = {
